...
def test_collect_repo_seeds_zero_snapshot_for_late_discovered_asset(tmp_path, monkeypatch):
    payloads = iter([
        [
            {
                "tag_name": "v1",
        ...
        [
            {
                ...
                "html_url": "https://example.test/v1",
                "assets": [
                    {"id": 1, "name": "app.js", "download_count": 11},
                    {"id": 2, "name": "app.css", "download_count": 5},
                ],
            },
        ],
    ])
    collection_times = iter(["2026-07-03T00:00:00Z", "2026-07-04T00:00:00Z"])
    monkeypatch.setattr("collector.fetch_releases", lambda repo, token=None: next(payloads))
    monkeypatch.setattr("collector.utc_now_iso", lambda: next(collection_times))
    ...
    with connect(db_file) as conn:
        rows = snapshots_for_repo(conn, "owner/repo", include_prereleases=True, asset_name="app.css")

    assert [(row["collected_at"], row["download_count"]) for row in rows] == [
        ("2026-07-01T00:00:00Z", 0),
        ("2026-07-04T00:00:00Z", 5),
    ]
